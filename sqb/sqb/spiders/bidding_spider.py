...
from sqb.urls import get_bidding
from bs4 import BeautifulSoup as soup

class BiddingSpider(CrawlSpider):
	name = 'bidding'
	url_dict_list = get_bidding()
	...
	def parse_list(self, response):
		...
		'''
		html = response.body
		logging.info('+++++++list: now crawling page: '+response.url)
		...
		for a in soup(str(html), 'lxml').find_all('a'):
			...
			logging.info('*******page: now crawling page: '+actual_url)
			yield Request(url = actual_url, meta = {'title':title, 'province':province, 'city':city, 'industry':server_type}, callback = self.parse_info)
